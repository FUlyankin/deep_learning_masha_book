...
\begin{equation*}
    \begin{aligned} 
      & W_1 = \begin{pmatrix} 1 & -1 \\ 0.5 & 0 \end{pmatrix} - \gamma \cdot \begin{pmatrix} -0.5 & 0 \\ -1 & 0 \end{pmatrix}\\
...
\begin{equation*}
    \begin{aligned} 
    & logloss = -(y_i \cdot \ln \hat p_i + (1 - y_i) \cdot \ln (1 - \hat p_i)) \\ 
...
Дальше алгоритм делается ровно так же, только в качестве стартового $d$ используется $logloss'_{\hat y},$ а не $MSE'_{\hat y}.$

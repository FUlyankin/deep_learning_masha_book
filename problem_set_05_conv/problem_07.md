...
Для свёрточных нейронных сетей действует точно такая же логика. Им надо увидеть объект целиком, тогда они сделают более точные выводы. 
...
Сначала оно $3 \times 3$, затем $5 \times 5$, затем $7 \times 7$ и так далее. Через $n$ свёрток поле обзора окажется $(2n + 1) \times (2n + 1)$. На каждом шаге свёртка убивает одну клетку сверху картинки и одну снизу картинки. Отсюда получается множитель $2$ перед $n$.
...
Можно подойти к решению этой задачки более сложно и выписать рекуррентное уравнение. Пусть $m_n$ -- это поле обзора после применения $n-$ой свёртки. Другими словами говоря, $m_n$ -- это исходный размер картинки. После применения первой свёртки он изменится. На каждом шаге у нас размерность картинки меняется по формуле 
...
Если мы применим свёртку размера $5 \times 5$ к картинке размера $13 \times 13$, она уменьшится до картинки $9 \times 9$. Затем получится картинка размера $5 \times 5$, а затем останется один пиксель. Получается, в случае свёртки $5 \times 5$ поле обзора изменяется по формуле 

$$
(4n + 1) \times (4n + 1).
$$

Множитель $4$ перед $n$ стоит, так как на каждом шаге картинка по вертикали и горизонтали сокращается на $4$ клетки. 
...
Какого размера должна быть картинка, чтобы после применения свёртки и пулинга её размерность упала до $10 \times 10$? 
...
На выходе получится картинка размера $3 \times 3$. За одну свёртку она превратится в один пиксель. Получается, что картинка $7 \times 7$ превращается в картинку $3 \times 3$. По аналогии картинка $15 \times 15$ будет превращаться в картинку $7 \times 7$. 
...
__ж)__ Пусть Маша делает после каждого свёрточного слоя нормализацию по батчам. Как это повлияет на поле обзора? А если Маша делает между слоями остаточные связи (skip-connection)? 

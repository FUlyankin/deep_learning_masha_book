...
- после третьего полносвязного слоя размер выхода будет $10$
- после применения softmax размер выхода не изменится
...
Если у изображения несколько каналов, к каждому из них мы применяем одни и те же веса, а затем делаем по ним суммирование
...
```{dropdown} Решение
Разберёмся с размерами выходов из каждого слоя:

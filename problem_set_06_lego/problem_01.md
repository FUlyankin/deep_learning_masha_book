...
Нейроны выплёвывают $x_1$ и $x_2$. Дальше мы складываем их с весами $w_1$ и $w_2$ и получаем выход $h$. 
...
```{dropdown} Решение
Чтобы посчитать прогноз на тесте для обычного дропаута, нам нужно умножить выход слоя на $1 - p$
...
Вероятность того, что не отключится ни один слой, составит 

$$
q = (1 - p^{k_1}) \cdot (1 - p^{k_2}) \cdot (1 - p^{k_3}).
$$

Получается, что хотя бы один слой умрёт с вероятностью $1 - q$. 
...
[^statia5]: [Self-Normalizing Neural Networks](https://arxiv.org/pdf/1706.02515.pdf)

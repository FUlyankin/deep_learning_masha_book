...
Маша ест конфеты и решает задачи по глубокому обучению. Число решённых задач $y$ зависит от числа съеденных конфет $x$. Если Маша не съела ни одной конфеты, она не хочет заниматься глубоким обучением. Поэтому для прогнозирования числа решённых задач по числу съеденных конфет можно использовать линейную модель с одним признаком без константы $y_i = w \cdot x_i.$

...
```{dropdown} Решение
Ряд не сходится (следует из интегрального признака Коши). Обобщённый гармонический ряд $\sum 1/n^{p} $ сходится при $ p > 1$ и расходится при $p \le 1$. Ряд $\eta_t^2 = \frac{1}{t^2}$ сходится. Значит, такая величина шага нам подходит.
...
```{dropdown} Решение
Не подойдёт. Ни сам ряд, ни его квадрат не сходятся по интегральному признаку Коши. 
```
...
```{dropdown} Решение
Не подойдёт. Оба ряда сходятся по интегральному признаку Коши. 
```

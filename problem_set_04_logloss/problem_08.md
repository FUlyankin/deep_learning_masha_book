...
__а)__  Маша построила для своей выборки распределение таргета, чтобы понять, насколько оно сбалансировано
...
Как думаете, зачем Маша сделала это?

```{dropdown} Решение
Маша увидела, что в выборке есть серьёзный дисбаланс. Первый класс встречается в три раза реже нулевого. Из-за этого Маша решила искусственно увеличить значение каждого слагаемого с $\ln p_i$ для первого класса в три раза. 
...
Важной частью всех детекторов изображений является решение задачи классификации для большого количества участков изображения. При этом многие из этих участков (патчей) содержат только фон, то есть преобладает класс «фон». Многие из изображений фона легко классифицируются как фон. 

То есть возникает много примеров одного из классов, которые при этом и так легко классифицируются моделью. Из-за этого при использовании логистической функции потерь мы неэффективно учимся. Введём обозначение

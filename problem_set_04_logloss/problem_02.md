...
Мы будем оптимизировать логистические потери, то есть 
...
__и)__ Предположим, что на последнем слое нашей нейросети мы используем softmax. В качестве функции потерь мы используем $\text{logloss}$. Как будет выглядеть производная $\frac{\partial \text{logloss}}{\partial z}$? 
...
На вход в softmax идёт $k$ элементов, на выходе получается $k$ элементов. Нам надо взять производную каждого по каждому. Получается, что производная softmax -- это матрица размера $k \times k.$ У нас будет две ситуации: $i = j$ и $i \ne j$. Начнём с $i = j$

...
```{dropdown} Решение
Если $T \to \infty$, тогда $e^{\tfrac{z_i}{T}} \to 1$. Получается, что в нашем векторе все три числа при бесконечном $T$ примут значение $1/3,$ то есть мы получим равномерное распределение. 
...
__в)__ Предположим, что объектов на порядок больше. Например, это реплики, которые Алиса может сказать вам в ответ на какую-то фразу.  Понятное дело, что вашей фразе будет релевантно какое-то подмножество ответов. Какие значения температуры сэмплирования $T$ смогут сделать реплики Алисы непредсказуемыми? А какие сделают их однотипными? 
...
При одних и тех же весах модели сэмплирование с высокой температурой будет более разнообразным, в том числе часто совершенно случайным, а сэмплирование с низкой температурой будет более устойчивым, чаще будет выдавать одно и то же при похожих входах.
